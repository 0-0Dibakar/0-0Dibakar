<h1 align="center">Dibakara Mahananda</h1>  

<p align="center">  
<strong>Full Stack Developer (MERN) | AI/ML Engineer |Wireless Communication| Open source contribution enthusiast</strong> 
</p>  

<p align="center">  
Solved <strong>1000+ DSA Problems</strong> on LeetCode 
</p>  

<p align="center">  
National Institute of Technology, Rourkela 
</p>  

<p align="center">  
<a href="https://leetcode.com/u/The_Walker_24/">     
<img src="https://img.shields.io/badge/LeetCode-1000%2B-success?style=for-the-badge&logo=leetcode&logoColor=yellow" />   
</a>   

<a href="mailto:dibakara024@email.com">     
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />   
</a>   

<a href="https://www.linkedin.com/in/dibakara-mahananda-949b90201/">     
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />   
</a> 
</p>  

---

## Engineering Profile  

I am a **Full Stack Developer (MERN)** and **AI/ML Engineer** with strong expertise in **data structures, algorithms, and scalable system design**.  

I have solved **1000+ problems on LeetCode**, reflecting strong algorithmic thinking and problem-solving skills.  

My interests lie at the intersection of **modern web engineering**, **machine learning**, **large language models (LLMs)**, and **AI hardware systems**. I enjoy building **end-to-end systems** — from intuitive user interfaces to cloud-deployed backend services, intelligent ML pipelines, and exploring **AI chip architecture for accelerating machine learning workloads**.  

I am also an **Open Source Contribution Enthusiast**, actively learning from and contributing to collaborative, production-grade codebases.  

---

## Core Competencies  

- **Full Stack Development (MERN)**  
  Building responsive, secure, and scalable web applications using MongoDB, Express.js, React.js, and Node.js.  

- **AI / Machine Learning Engineering**  
  Designing and implementing NLP systems, LLM-powered applications, and deep learning pipelines using modern ML frameworks.  

- **AI Chip Design & Hardware for AI**  
  Exploring hardware architectures optimized for AI workloads including **neural network accelerators, parallel compute architectures, and hardware-efficient ML systems**.  

- **Data Structures & Algorithms**  
  Strong command over DSA, competitive programming, and problem-solving with 1000+ solved problems.  

- **Cloud & Deployment**  
  Deploying and maintaining full-stack and ML applications on AWS using industry best practices.  

- **Open Source Collaboration**  
  Enthusiastic about contributing to open-source projects, understanding large codebases, and following clean engineering practices.  

---

## Technical Arsenal  

| Domain | Stack |
|------|------|
| Programming Languages | C++, Python, JavaScript (ES6+), HTML5, CSS3, SQL |
| Frontend Engineering | React.js, Modern UI Architecture, Responsive Design |
| Backend Engineering | Node.js, Express.js, RESTful & API-driven Systems |
| Databases | MongoDB, SQL-based Relational Databases |
| Artificial Intelligence | Machine Learning, Deep Learning, NLP, LLMs |
| ML Frameworks | TensorFlow, PyTorch, JAX |
| Data Processing | Pandas, NumPy, Data Cleaning & Feature Engineering |
| Cloud Platforms | AWS (EC2, S3, IAM), Cloud-Native Fundamentals |
| DevOps Practices | Docker (Foundational), CI/CD Concepts |
| Engineering Tools | Git, GitHub, Linux, API Integration |
| **AI Chip Design** | Computer Architecture Fundamentals, Parallel Computing, AI Accelerators |
| **Hardware Description Languages** | Verilog (Learning), SystemVerilog (Basics) |
| **VLSI Design Concepts** | Digital Logic Design, RTL Design, FPGA Fundamentals |
| **Hardware Tools** | Vivado (FPGA), ModelSim (Simulation) |
| **AI Hardware Concepts** | Systolic Arrays, Matrix Multiplication Accelerators, TPU/GPU Architecture Basics |

---

## Open Source & Community  

- Open source contribution enthusiast  
- Actively exploring large-scale repositories  
- Passionate about clean code, documentation, and collaborative development  
- Interested in contributing to **AI/ML**, **developer tooling**, **AI hardware**, and **web infrastructure** projects  

---

<p align="center">  
<i>“Strong fundamentals, clean code, scalable systems, and efficient AI hardware.”</i>  
</p>
